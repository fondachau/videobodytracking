		...
		for (;;)
		{
			...
			if (Is == false) {
				...
				if (prevgray.empty() == false) {
					...
					for (int y = toprange; y < bottomrange; y += 10) {
						for (int x = leftrange; x < rightrange; x += 10)
						{
					...
					printf("\n 0");
					Mat frame1=original;
					Mat inpBlob = blobFromImage(frame1, 1.0 / 255, Size(inWidth, inHeight), Scalar(0, 0, 0), false, false);
